...
import com.sf.blog.dao.CategoryDao;
...
public class AddCategoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            //create category constructor
            Category cat = new Category(name, desc);

            //category story in the db
            CategoryDao cdao = new CategoryDao(ConnectionProvider.getConnection());

            if (cdao.getCatgoryAddByAdmin(cat)) {

                HttpSession s = request.getSession();
                s.setAttribute("message", "Categery Successfully Added");
                response.sendRedirect("admin_dashboard.jsp");

            } else {
                HttpSession s = request.getSession();
                s.setAttribute("message", "Something Want Wrong");
                response.sendRedirect("admin_dashboard.jsp");
            }
